...
- **NEW (#50)**: Added deterministic session auditing via the direct `./.session/scripts/bash/session-audit.sh` CLI script — audits recorded sessions by workflow/date/session filters, reports workflow adherence/artifact completeness/task completion/validation/handoff health, and uses durable session artifacts plus available local bookkeeping with graceful degradation when local-only files are missing
- **FIX (#50)**: `session-validate.sh` now persists schema-versioned validation summaries to both `{session_dir}/validation-results.json` and the latest local `.session/validation-results.json`, so publish and historical audits consume the same deterministic validation data
