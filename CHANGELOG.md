...
- **NEW (#50)**: Added deterministic session auditing via `session-audit.sh` / `invoke session.audit` — audits recorded sessions by workflow/date/session filters, reports workflow adherence/artifact completeness/task completion/validation/handoff health, and uses durable session artifacts plus available local bookkeeping with graceful degradation when local-only files are missing
